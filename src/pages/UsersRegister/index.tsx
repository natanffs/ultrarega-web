...
const UsersRegister: React.FC = () => {
    ...
    const [permissions, setPermissions] = useState<permissionI[]>(JSON.parse(localStorage['permissions_user']))
            ...
            <Wrapper>
                ...
                <FormUtr >
                    ...
                    {permissions.length > 0 && permissions.map((p: permissionI) =>
                        <Utrs key={p.id}>
                            <WrapperCheckBox>
                                <CheckBox type="checkbox" value={p.id} onChange={(text: React.ChangeEvent<HTMLInputElement>) => {
                                    if (text.target.checked) {
                                        setSelectedPermissions([...selectedPermissions, Number(p.id)])
                                    } else {
                                        setSelectedPermissions(selectedPermissions.filter(su => su !== Number(p.id)))
                                    }
                                }} />
